...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(self, x, timesteps, context, control_input):
        """
        Args:
        ...
        """
        # Concatenate input image with control input
        x = torch.cat([x, control_input], dim=1)
        
        # Initial convolution
        hidden_states = self.conv_in(x)
        
        # Store controlled outputs
        ...
        # Apply zero convolutions to generate control signals
        controlled_outputs = []
        
        for down_block_res_sample, controlnet_block in zip(
            down_block_res_samples, self.controlnet_down_blocks
        ):
            controlled_outputs.append(controlnet_block(down_block_res_sample))
        
        controlled_outputs.append(self.controlnet_mid_block[0](hidden_states))
...
class ControlNet(nn.Module):
    def __init__(self, unet):
        ...
        # Input convolution
        self.conv_in = nn.Conv2d(
            unet.conv_in.in_channels + 3,  # Add 3 channels for control input
        ...
        time_embed_dim = unet.config.block_out_channels[0] * 4

        # Copy down blocks from UNet
        self.down_blocks = nn.ModuleList([])
        for down_block in unet.down_blocks:
            if isinstance(down_block, CrossAttnDownBlock2D):
                new_block = CrossAttnDownBlock2D(
                    in_channels=down_block.resnets[0].in_channels,
                    out_channels=down_block.resnets[0].out_channels,
                    temb_channels=time_embed_dim,
                    num_layers=len(down_block.resnets),
                    resnet_eps=1e-5,
                    resnet_act_fn="silu",
                    attention_head_dim=64,  # Standard size
                    cross_attention_dim=unet.config.cross_attention_dim,  # From config
                    downsample_padding=1,
                    resnet_time_scale_shift="default"
                )
            else:
                new_block = DownBlock2D(
                    in_channels=down_block.resnets[0].in_channels,
                    out_channels=down_block.resnets[0].out_channels,
                    temb_channels=time_embed_dim,
                    num_layers=len(down_block.resnets),
            ...
            self.down_blocks.append(new_block)

        # Copy middle block from UNet
        self.mid_block = UNetMidBlock2DCrossAttn(
            in_channels=unet.mid_block.resnets[0].in_channels,
            temb_channels=time_embed_dim,
            resnet_eps=1e-5,
            resnet_act_fn="silu",
            ...
            resnet_time_scale_shift="default",
            cross_attention_dim=unet.config.cross_attention_dim,  # From config
            num_attention_heads=8,  # Standard size instead of attention_head_dim
            num_layers=1,
            resnet_groups=32
        )

        # Zero Convolutions for controlled outputs
        self.controlnet_down_blocks = nn.ModuleList([])
        
        # Add zero convs for down blocks
        for down_block in self.down_blocks:
        ...
        # Add zero conv for middle block
    ...
    def forward(self, x, emb, context, control_input):
        """
        Args:
            x: Input tensor
            ...
            control_input: Control signal (e.g., edge map)
        """
        # Concatenate input with control input
        x = torch.cat([x, control_input], dim=1)
